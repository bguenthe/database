...
select *
from mqtt_logger
where lower(payload) like '%tex%'

SELECT payload::json ->> 'id'             id,
       payload::json ->> 'uniqueID'       uniqueid,
       payload::json ->> 'recordDateTime' recordDateTime,
       payload::json ->> 'costs'          costs,
       payload::json ->> 'deleted'        "deleted"
from mqtt_logger
where topic = 'monthlycosts/clientcosts'
  and payload::json ->> 'manmod' = 'samsung_SM-S928B'
...
SELECT payload::json ->> 'uniqueID', payload::json ->> 'recordDateTime', count(*)
from mqtt_logger
where topic = 'monthlycosts/income'
  and payload::json ->> 'manmod' = 'samsung_SM-S928B'
group by payload::json ->> 'uniqueID', payload::json ->> 'recordDateTime'
having count(payload::json ->> 'uniqueID') > 1;
...
SELECT *,
       to_number(payload::json ->> 'costs', '99999.99') costs,
       payload::json ->> 'comment'                      "comment",
       to_date(payload::json ->> 'recordDateTime', 'YYYY-MM-DD')
from mqtt_logger
where topic = 'monthlycosts/clientcosts'
  and payload::json ->> 'manmod' = 'samsung_SM-S928B'
...
create table mqtt_logger_save as
select *
from mqtt_logger;
...
delete
from mqtt_logger
where payload::json ->> 'id' in ('1684', '1682', '1681')
...
with monthlyincome as (select substr(payload::json ->> 'recordDateTime', 1, 7)    ym,
                              sum((payload::json ->> 'income')::DOUBLE PRECISION) sum
                       from mqtt_logger
                       where topic like '%clientincome%'
                         and payload::json ->> 'manmod' = 'samsung_SM-S928B'
                         and payload::json ->> 'deleted' = 'false'
...
with monthlyincome as (select sum((payload::json ->> 'income')::DOUBLE PRECISION) sum
                       from mqtt_logger
                       where topic like '%clientincome%'
                         and payload::json ->> 'manmod' = 'samsung_SM-S928B'
                         and payload::json ->> 'deleted' = 'false'),
     monthlycosts as (select sum((payload::json ->> 'costs')::DOUBLE PRECISION) sum
                      from mqtt_logger
                      where topic like '%clientcosts%'
                        and payload::json ->> 'deleted' = 'false'
                        and payload::json ->> 'manmod' = 'samsung_SM-S928B')
select monthlyincome.sum, monthlycosts.sum, ((monthlyincome.sum - monthlycosts.sum) / 48) - 797
from monthlyincome
         join monthlycosts on 1 = 1;

-- Income 138160 in 48 Monaten
-- Costs  081561 in 48 Monaten

/* Monatlicher Durchschnitt */
with monthcount as (select payload::json ->> 'type', sum(to_number(payload::json ->> 'costs', '9999999.99')) / count(*)
                    from mqtt_logger
                    where 1 = 1
                      and payload::json ->> 'deleted' = 'false'
                      and topic = 'monthlycosts/clientcosts'
...
select max(to_date(payload::json ->> 'recordDateTime', 'YYYY-MM-DD')),
       min(to_date(payload::json ->> 'recordDateTime', 'YYYY-MM-DD'))
from mqtt_logger
where 1 = 1
  and payload::json ->> 'deleted' = 'false'
  and topic = 'monthlycosts/clientcosts'
